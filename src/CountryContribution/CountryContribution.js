...
function CountryContribution({ countries, totalNumbers }) {
  ...
  return (
    <table className='container-table-country-data'>
      <thead>
        <tr>
          <th style={{ border: '1px solid #303030', borderRight:"1px solid #303030", padding: '8px' }}>Country</th>
          <th style={{ border: '1px solid #303030', padding: '8px', cursor: 'pointer' ,borderRight:"1px solid #303030"}} onClick={toggleSortOrder}>
            Number of Numbers
            {sortOrder === 'asc' ? ' ↑' : ' ↓'}
          </th>
          <th style={{ border: '1px solid #303030', padding: '8px' }}>Contribution (%)</th>
        </tr>
      </thead>
      <tbody>
        {sortedCountries?.map((country, index) => {
          ...
          return (
            <tr key={index} style={{ overflowY: 'auto' }}>
              ...
              <td style={{ border: '1px solid #303030', padding: '8px' }}>{country.numbers.toLocaleString()}</td>
              <td style={{ border: '1px solid #303030', padding: '8px' }}>{contribution.toFixed(2)}%</td>
            </tr>
          );
        })}
      </tbody>
